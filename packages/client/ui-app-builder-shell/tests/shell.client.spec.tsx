...
/**
 * App Builder shell renderer: a 3-pane CSS-Grid layout. The shell owns the
 * selection store and threads the current `selectedProjectId` into the
 * projects slot owner prop on every renderSlot call. This suite exercises
 * the four branches that drive the shell through props alone (the
 * SlotRegistry wiring in `apply()` is out of scope; the renderer only
 * reads the four prop shares).
 *
...
interface RenderSlotCall {
  slot: 'app-builder.projects' | 'app-builder.preview' | 'app-builder.conversation'
  owner: unknown
}
...
function makeRenderSlotStub(): {
  ...
  renders: Record<RenderSlotCall['slot'], number>
} {
  const calls: RenderSlotCall[] = []
  const renders: Record<RenderSlotCall['slot'], number> = {
  ...
  const renderSlot = ((slot: RenderSlotCall['slot'], owner: unknown) => {
    if (slot !== 'app-builder.projects' && slot !== 'app-builder.preview' && slot !== 'app-builder.conversation') {
...
describe('App Builder shell renderer', () => {
  it('renders the localized header title and the three pane slots', () => {
