 ...
 * (`@deepseek-ai/dsh-app-builder-deployment`). `getUsage` remains
 * deferred until Phase 2.3 adopts token / cost accounting sourced from
 * `@deepseek-ai/dsh-tool-policy`. The deferred methods return a typed
 * `not-implemented` failure so a missing owner never silently succeeds;
 * the gateway surfaces the typed rejection to the client (the Agent / UI
 * distinguishes `not-implemented` from `internal` failures on the same path).
...
/**
 * `getUsage` placeholder. The real implementation lands when Phase 2.3
 * adopts the token / cost accounting policy package; today it returns
 * the typed `not-implemented` failure so callers can wire against the
 * method today.
...
export async function getUsageRemote(_ctx: Context, _request: GetUsageRequest): Promise<GetUsageValue> {
  await Promise.resolve()
  throw notImplemented('getUsage', 'token / cost accounting is Phase 2.3 deferred (no @deepseek-ai/dsh-tool-policy in tree yet)')
}
