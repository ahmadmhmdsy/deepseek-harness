...
| `apps/cli/config/examples/app-builder` | **shipped** (`dd8a6a399b`, this step); **relocated in Phase 1.5 (1.5.2)** | keyless smoke (mock LLM) passes; with-key smoke is `describe.skipIf(!DEEPSEEK_API_KEY)`; `cordis.yml` inlines project+scaffold+preview plugins; persona pulled via `agent-spine.config.persona` `!!js createRequire` indirection (avoids persona-plugin / deployment-persona collision); agent-spine loads BEFORE the App Builder plugins so `agents` (the `AgentRegistry` service) is published first; originally at `examples/app-builder/` until Phase 1.5 (1.5.2) moved it under `apps/cli/config/examples/` to match upstream's post-PR #2977 layout |
...
- The `index.ts` for the scaffold package carries `/* v8 ignore */` not used; the full plugin lifecycle (including `ctx.tools.register`, `ctx.fs.writeText`, the optional `ctx.jobs.start` branch) is reachable only through the Loader-driven smoke in `apps/cli/config/examples/app-builder/`, which the next commits build out (the example lived at `examples/app-builder/` until Phase 1.5 (1.5.2) relocated it).
