...
Per-candidate scope dedup applies equally to a `packages/CLAUDE.md` that points at `../CLAUDE.md` and to one that points at local `AGENTS.md`: the resolved content is what dedupes, not the path geometry. The repo's current per-folder layout — root `CLAUDE.md` as a regular file, `packages/CLAUDE.md` and `examples/CLAUDE.md` symlinking to `../CLAUDE.md`, `vendor/CLAUDE.md` and `.agents/notes/implemented/CLAUDE.md` symlinking to local `AGENTS.md` — is recorded in [Promote root CLAUDE.md to the canonical agent operating system](2026-08-29-claude-md-operating-system.md). That note partially supersedes the implicit assumption that CLAUDE.md should mirror AGENTS.md everywhere; the dedup mechanism itself is unchanged.
