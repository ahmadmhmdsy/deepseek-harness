...
- **English-only documentation.** English sources are canonical since Phase 1.5.7. Do not create `*.zh.md` files or re-record `*.i18n.yaml` sidecars; Chinese files stay frozen. The [pairing workflow](i18n/README.md) and `dsh-translate-docs` are dormant; EN/ZH divergence is §9 backlog.
